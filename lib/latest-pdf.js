...
import { loadRuntimeComics } from "./runtime-comics.js";
...
export async function readLatestRuntimePdfFromBucket({ loadComics, bucket }) {
  if (typeof loadComics !== "function") throw new TypeError("loadComics must be a function");
  return readLatestPdfFromBucket(await loadComics(), bucket);
}
...
export function createApp(env = process.env, options = {}) {
  ...
  const loadComics = options.loadComics || (() => loadRuntimeComics({
    bucket,
  ...
  for (const endpointPath of ENDPOINT_PATHS) {
    app.get(endpointPath, async () => {
      try {
        const { comic, object } = await readLatestRuntimePdfFromBucket({ loadComics, bucket });
